...
const schema = Joi.object({
  calibration_id: Joi.string().min(2).max(30).required(),
  sold_to_party: Joi.string().min(2).max(30).required(),
  contract_no: Joi.string().min(2).max(20).required(),
  make: Joi.string().min(1).max(30).required(),
  model: Joi.string().min(1).max(30).required(),
  id_num: Joi.number().min(1).max(20).required(),
  current_stamping_date: Joi.date().required(),
  calibration_start_date: Joi.date().required(),
  calibration_end_date: Joi.date().required(),
...
router.get("/allImages", async (req, res) => {
  try {
    const result = await Products.find().select({ images: 1, _id: 0 });
    const imgs = [];
    result.forEach((r) => imgs.push(r?.images[0]));
    // Products.find({})
    //   .select("images")
...
router.get("/brandsByCategory/", async (req, res) => {
  try {
    ...
    brands = await Products.distinct("brand")
      .where("category")
      .equals(category);
...
router.get("/brands", async (req, res) => {
  try {
    let uniqueBrands;
    uniqueBrands = await Products.distinct("brand");
    return res.send({
      brands: uniqueBrands,
    });
  } catch (err) {
...
router.get("/productsByCategory/", async (req, res) => {
  try {
    ...
    products = await Products.find().where("category").equals(category);
...
router.get("/productsByBrands/", async (req, res) => {
  try {
    const brand = req.query.brand;
    let products;

    products = await Products.find().where("brand").equals(brand);
...
router.get("/productsByCategoryBrands/", async (req, res) => {
  try {
    const category = req.query.category;
    const brand = req.query.brand;
    const products = await Products.find({ category: category, brand: brand });
